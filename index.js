...
function nextNewYear() {
    ...
    newYearDate.setFullYear(year + 1, 0, 1);
...
function isLeapYear(year) {
    ...
    if (year % 4 === 0) {
        return { year: year, isLeap: Date.parse(year) % 4 === 0 };
    } else {
        return false;
    }
...
function addDays(date, days) {
    ...
    //const resultDate = inputDate + 1000 * 60 * 60 * 24;
    // Перевірка, чи є вхідне значення об'єктом Date,це можно зробити перевіривши чи є date.getTime по типу функція .
    // Якщо date не є об'єктом Date, повертаємо рядок
    // "Помилка: вхідне значення має бути об'єктом Date"
...
function getDaysInMonth(date) {
    ...
    const dateWithNextMonth = new Date(currentYear, currentMonth + 1, 0);
    const daysInMonth = dateWithNextMonth.getDate();
    return {
...
function getDaysDifference(startDate, endDate) {
    ...
    const diffInDay = (endTime - startTime) / (1000 * 60 * 60 * 24);
    return {
        startDate: startDate.toISOString(),
        endDate: endDate.toISOString(),
        daysDifference: diffInDay,
    };
    // Перевірка, чи є вхідні значення об'єктами Date.
